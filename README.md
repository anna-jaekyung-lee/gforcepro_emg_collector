...
The ADC zero-point varies per channel and per session — resting EMG is not naturally centered at 0. This tool optionally collects a user-defined REST period at recording start, computes the per-channel mean, and subtracts it from all subsequent samples. Signals are then centered around 0 during rest.
...
- **16-bit resolution** — explore whether firmware supports `resolution=16` via SDK command; currently unverified
- **IMU separate file** — store IMU at its native rate in a separate CSV, merge with EMG by timestamp for analysis
- **Filter validation** — FFT/PSD-based verification of filter effectiveness across recording conditions
